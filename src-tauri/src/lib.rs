...
type SidecarHandleSetup = SidecarHandle;
#[cfg_attr(mobile, tauri::mobile_entry_point)]
pub fn run() {
    let sidecar_handle: SidecarHandle = Arc::new(Mutex::new(None));
    let sidecar_handle_setup: SidecarHandleSetup = sidecar_handle.clone();
    tauri::Builder::default()
        .plugin(tauri_plugin_shell::init())
        .manage(sidecar_handle)
        .invoke_handler(tauri::generate_handler![get_status, trigger_panic])
        .setup(move |app| {
